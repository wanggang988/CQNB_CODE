...
#include "../include/nmea/parser.h"
#include "../include/nmea/context.h"
#include <rtthread.h>
#include <string.h>
#include <stdlib.h>
...
int nmea_parser_real_push(nmeaPARSER *parser, const char *buff, int buff_sz)
{
    ...
    //HAL_Delay(10);
    memcpy(parser->buffer + parser->buff_use, buff, buff_sz);//内存拷贝函数
    //memcpy(parser->buffer, buff, buff_sz);//内存拷贝函数
    //memcpy(parser->buffer [ parser->buff_use], buff, buff_sz);//内存拷贝函数
    ...
    for(;;node = 0)
    {
        ...
        else if(crc >= 0)
        {
            ...
            case GPVTG:
                ...
                break;
            case GNGGA:
                if(0 == (node->pack = rt_malloc(sizeof(nmeaGNGGA))))
                    goto mem_fail;
                node->packType = GNGGA;
                if(!nmea_parse_GNGGA(
                    (const char *)parser->buffer + nparsed,
                ...
                break;
            case GNRMC:
                if(0 == (node->pack = rt_malloc(sizeof(nmeaGNRMC))))
                  goto mem_fail;
            ...
            case GNVTG:
                if(0 == (node->pack = rt_malloc(sizeof(nmeaGNVTG))))
                    goto mem_fail;
                node->packType = GNVTG;
                if(!nmea_parse_GNVTG(
                    (const char *)parser->buffer + nparsed,
                ...
                break;
            case GNZDA:
                if(0 == (node->pack = rt_malloc(sizeof(nmeaGNZDA))))
                    goto mem_fail;
                node->packType = GNZDA;
                if(!nmea_parse_GNZDA(
                    (const char *)parser->buffer + nparsed,
                ...
                break;
            case GNGLL:
                if(0 == (node->pack = rt_malloc(sizeof(nmeaGNGLL))))
                  goto mem_fail;
                ...
                break; 
            case BDGSV:
                if(0 == (node->pack = rt_malloc(sizeof(nmeaBDGSV))))
                  goto mem_fail;
                ...
                break; 
            case BDGSA:
                if(0 == (node->pack = rt_malloc(sizeof(nmeaBDGSA))))
                  goto mem_fail;
            ...
            case GPTXT:
                if(0 == (node->pack = rt_malloc(sizeof(nmeaGPTXT))))
                  goto mem_fail;
